...
async function clearConfigs(secretsManager) {
    let res = await secretsManager.getConfig({secretType: SecretsManager.CreateSecretConfigElementConstants.SecretType.PUBLIC_CERT});
    for (let i = 0; i < res.result.resources[0].certificate_authorities.length; i++) {
        let c = res.result.resources[0].certificate_authorities[i];
        const res2 = await secretsManager.deleteSecretConfigElement(
            {
                secretType: SecretsManager.CreateSecretConfigElementConstants.SecretType.PUBLIC_CERT,
                configElement: SecretsManager.DeleteSecretConfigElementConstants.ConfigElement.CERTIFICATE_AUTHORITIES,
                configName: c.name
            });
    ...
    for (let i = 0; i < res.result.resources[0].dns_providers.length; i++) {
        let c = res.result.resources[0].dns_providers[i];
        const res3 = await secretsManager.deleteSecretConfigElement(
            {
                secretType: SecretsManager.CreateSecretConfigElementConstants.SecretType.PUBLIC_CERT,
                configElement: SecretsManager.DeleteSecretConfigElementConstants.ConfigElement.DNS_PROVIDERS,
                configName: c.name
            });
...
describe('IbmCloudSecretsManagerApiV1_integration', () => {
  ...
  beforeAll( async () => {
    ...
    test('Should create configs, order certificate and delete configs', async () => {
        ...
        //Create CA config
        let res = await secretsManager.createSecretConfigElement({
            secretType: SecretsManager.CreateSecretConfigElementConstants.SecretType.PUBLIC_CERT,
            configElement: SecretsManager.CreateSecretConfigElementConstants.ConfigElement.CERTIFICATE_AUTHORITIES,
            name: caConfigName,
        ...
        res = await secretsManager.createSecretConfigElement({
            secretType: SecretsManager.CreateSecretConfigElementConstants.SecretType.PUBLIC_CERT,
            configElement: SecretsManager.CreateSecretConfigElementConstants.ConfigElement.DNS_PROVIDERS,
            name: dnsConfigName,
            type: 'cis',
        ...
        const id = res.result.resources[0].id;
        ...
        //delete the configs
        res = await secretsManager.deleteSecretConfigElement(
            {
                secretType: SecretsManager.DeleteSecretConfigElementConstants.SecretType.PUBLIC_CERT,
                configElement: SecretsManager.DeleteSecretConfigElementConstants.ConfigElement.DNS_PROVIDERS,
    ...
    test('Should create, get, list and delete configs', async () => {
        ...
        //get ca config
        res = await secretsManager.getSingleSecretConfigElement(
            {
                secretType: SecretsManager.CreateSecretConfigElementConstants.SecretType.PUBLIC_CERT,
                configElement: SecretsManager.DeleteSecretConfigElementConstants.ConfigElement.CERTIFICATE_AUTHORITIES,
                configName: caConfigName
        ...
        res = await secretsManager.getSingleSecretConfigElement(
            {
                secretType: SecretsManager.GetSecretConfigElementConstants.SecretType.PUBLIC_CERT,
                configElement: SecretsManager.GetSecretConfigElementConstants.ConfigElement.DNS_PROVIDERS,
                configName: dnsConfigName
            });
        expect(res.status).toBe(200);
        ...
        //Get all configs
        res = await secretsManager.getConfig({secretType: SecretsManager.CreateSecretConfigElementConstants.SecretType.PUBLIC_CERT});
        expect(res.status).toBe(200);
