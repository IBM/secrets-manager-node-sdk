...
async function clearConfigs(secretsManager) {
    let res = await secretsManager.getConfig({secretType: SecretsManager.GetConfigConstants.SecretType.PUBLIC_CERT});
        ...
        const res2 = await secretsManager.deleteConfigElement(
            {
                ...
                configElement: SecretsManager.DeleteConfigElementConstants.ConfigElement.CERTIFICATE_AUTHORITIES,
                configName: c.name
            });
...
describe('IbmCloudSecretsManagerApiV1_integration', () => {
    ...
    test('Should create, get, list and delete configs', async () => {
        ...
        //Get all configs
        res = await secretsManager.getConfig({secretType: SecretsManager.GetConfigElementConstants.SecretType.PUBLIC_CERT});
        expect(res.status).toBe(200);
